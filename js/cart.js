  ...
  function getLSContent() {
    ...
    const lsContent = JSON.parse(localStorage.getItem("NileFish")) || [];
    return lsContent;
  }

  function setLSContent(lsContent) {
    // save content inside local storage
    localStorage.setItem("NileFish", JSON.stringify(lsContent));
  }
  ...
  function getCartItemPrices() {
    ...
    let counts = cartContent.querySelectorAll("tr td:nth-child(3)");
    ...
    if (nums.length > 0) {
      for (let cell = 0; cell < nums.length; cell++) {
        ...
        let count = counts[cell].innerText;
        num = num.replace(/[^\d]/g, "");
  ...
  function saveProduct(clickedBtn) {
    ...
    const prodName = cardInfo.querySelector("#product-name").textContent;
    const prodCount = cardInfo.querySelector("#count").value;
    const prodPrice = cardInfo.querySelector("#product-price").textContent;
